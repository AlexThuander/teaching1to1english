    		...
    		<div class="form-group col-md-4">
	            ...
	            <label class="cabinet center-block">
	                <figure class="course-image-container">
	                    ...
	                    <img src="@if(Storage::exists($instructor->instructor_image)){{ Storage::url($instructor->instructor_image) }}@else{{ asset('backend/assets/images/female_profile.png') }}@endif" class="gambar img-responsive" id="course_image-output" name="course_image-output" />
	                	<input type="file" class="item-img file center-block" name="course_image" id="course_image" />
	                </figure>
                    <span style="font-size: 10px;">
                    Supported File Formats: jpg,jpeg,png 
                    <br>Dimesnion: 258px X 258px
...
<script type="text/javascript">
    ...
    { 
        //image crop start
        $(".gambar").attr("src", @if(Storage::exists($instructor->instructor_image))"{{ Storage::url($instructor->instructor_image) }}" @else "{{ asset('backend/assets/images/female_profile.png') }}" @endif);
                ...
                var data_content = this_id.attr('data-content');
                 $.ajax({
                    ...
                    success: function (data) {
                        $("#"+current_id+"-output").attr("src", "{{ asset('backend/assets/images/female_profile.png') }}");
                        this_id.hide();
                    }
                });
            }, function () {    
